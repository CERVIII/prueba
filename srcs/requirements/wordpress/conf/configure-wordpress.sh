...
then
    ...
    wp-cli.phar core install --url=https://${NGINX_HOST} --title="${WP_TITLE}" --admin_user=${WP_ADMIN_USER} --admin_password=${WP_ADMIN_PASS} --admin_email=${WP_ADMIN_EMAIL} --path=${WP_PATH} --allow-root
    echo "[WP config] Configuring WordPress to use HTTPS..."
    wp-cli.phar option update home "https://${NGINX_HOST}" --path=${WP_PATH} --allow-root
